...
------- ACTION MESSAGE -------
    elseif e.id == 0x29 then
        ...
        elseif passed_messages:contains(am.message_id) then
            ...
            :gsub('${actor}\'s',actor_article..gFuncs.ColorIt(actor.name or '',gProfileColor[actor.owner or actor.type])..'\'s'..(actor.owner_name or "???"))
            :gsub('${actor}',actor_article..gFuncs.ColorIt(actor.name or '',gProfileColor[actor.owner or actor.type])..(actor.owner_name or "???"))
            :gsub('${status}',status or '')
            :gsub('${item}',gFuncs.ColorIt(item or '',gProfileColor.itemcol))
            :gsub('${target}\'s',target_article..gFuncs.ColorIt(target.name or '',gProfileColor[target.owner or target.type])..'\'s'..(target.owner_name or "???"))
            :gsub('${target}',target_article..gFuncs.ColorIt(target.name or '',gProfileColor[target.owner or target.type])..(target.owner_name or "???"))
