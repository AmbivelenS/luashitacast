...
gcinclude.settings = {
	...
	WScheck = false; --set to false if you dont want to use the WSdistance safety check
...
gcinclude.AliasList = T{'gcmessages','wsdistance','dt','th','kite','meleeset','gcdrain','gcaspir','nukeset','burst','weapon','elecycle','helix','weather','nuke','death','fight','sir','tankset','proc','cj','pupmode','tpgun','cormsg','forcestring','siphon','warpring','telering','rrset','craftset','zeniset','fishset', 'expband','meleestaff'};
...
gcinclude.ElementalStaves ={
    ['Fire'] = 'Kukulcan\'s Staff',--'Fire Staff',
    ['Earth'] ='Kukulcan\'s Staff', --'Terra\'s Staff',
    ['Water'] = 'Kukulcan\'s Staff',--'Water Staff',
    ['Wind'] = 'Wind Staff',--'Water Staff',
    ['Ice'] = 'Kukulcan\'s Staff',--'Aquilo\'s Staff',
    ['Thunder'] = 'Kukulcan\'s Staff',--'Thunder Staff',
    ['Light'] ='Light Staff', --'Light Staff',
    ['Dark'] = 'Dark Staff',--'Pluto\'s Staff'
};
gcinclude.Summons = {
    ['Ifrit'] = 'Kukulcan\'s Staff',--'Fire Staff',
    ['Titan'] ='Kukulcan\'s Staff', --'Terra\'s Staff',
    ['Leviathan'] = 'Kukulcan\'s Staff',--'Water Staff',
    ['Garuda'] = 'Wind Staff',--'Water Staff',
    ['Shiva'] = 'Kukulcan\'s Staff',--'Aquilo\'s Staff',
    ['Ramuh'] = 'Kukulcan\'s Staff',--'Thunder Staff',
    ['Carbuncle'] ='Light Staff', --'Light Staff',
	['Fenrir'] = 'Dark Staff',--'Pluto\'s Staff'
    ['Diablos'] = 'Dark Staff',--'Pluto\'s Staff'
};
...
gcinclude.ExpBand = false;
gcinclude.WarpClub = false;
gcinclude.MeleeStaff = false;
...
function gcinclude.SetCommands(args)
	...
	elseif (args[1] == 'warpring') then
		...
		gcinclude.WarpClub = not gcinclude.WarpClub;
		toggle = 'warpring';
		status = gcinclude.WarpClub;
	...
	elseif (args[1] == 'expband') then
		gcinclude.ExpBand = not gcinclude.ExpBand;
		toggle = 'ExpBand';
		status = gcinclude.ExpBand;
	elseif (args[1] == 'meleestaff') then
		gcinclude.MeleeStaff = not gcinclude.MeleeStaff;
		toggle = 'meleestaff';
		status = gcinclude.MeleeStaff;
    end
	if (player.MainJob == 'RDM') or (player.MainJob == 'BLM') or (player.MainJob == 'SCH') or (player.MainJob == 'GEO') then
...
function gcinclude.DoWarpRing()
	AshitaCore:GetChatManager():QueueCommand(1, '/lac equip Main "'.. gcinclude.settings.WarpItem ..'"');

	local function usering()
		local function forceidleset()
			AshitaCore:GetChatManager():QueueCommand(1, '/lac set Idle');
		end
		AshitaCore:GetChatManager():QueueCommand(1, '/item "'.. gcinclude.settings.WarpItem ..'" <me>');
		forceidleset:once(8);
	end
...
function gcinclude.CheckDefault()
	...
	if (gcinclude.ExpBand == true) then gFunc.EquipSet(gcinclude.sets.ExpBand) end
	if (gcinclude.RRSET == true) then gFunc.EquipSet(gcinclude.sets.Reraise) end
	if (gcinclude.WarpClub == true) then gFunc.EquipSet(gcinclude.sets.WarpClub) end
...
--Custom LevelSync Function
function gcinclude.levelSync(levelOffset)
	local player = gData.GetPlayer();
	local mainJobLevel = player.MainJobLevel;
	local mainJobSync = player.MainJobSync;
	local syncData;
	
	if (mainJobSync < mainJobLevel and mainJobSync < levelOffset) then
		--Level Sync Gear
		syncData = 1 --'Sync_';
